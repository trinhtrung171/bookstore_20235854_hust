...
// =========================
let currentPath = '/';
let mainContentMode = 'home'; // NEW STATE: 'home' hoặc 'search' để quản lý nội dung chính
...
// NEW: Trạng thái cho việc tìm kiếm, thay thế cho window.__searchState
let searchState = {
    name: '',
    author: '',
    category: '',
    ...
    maxPrice: 1000000,
    results: [],
    hasSearched: false // Cờ để biết đã thực hiện tìm kiếm hay chưa
};
...
let availableBanners = []; // Mảng lưu banner
...
let isAddVoucherModalOpen = false; // cho voucher
let editVoucherModalState = { isOpen: false, voucher: null };



// BIẾN STATE MỚI CHO FLASH SALE MODAL
...
// =========================
// Danh sách sách (Dùng chung cho trang chủ và kết quả tìm kiếm)
// =========================
...
// Thêm hàm xử lý thêm vào yêu thích (cần thêm bảng/favorite nếu chưa có)
...
// NEW: Hàm tạo giao diện trang chủ mặc định
...
// NEW: Hàm tạo giao diện tìm kiếm
...
// NEW: Hàm khởi tạo thanh trượt giá
...
// MODIFIED: Hàm xử lý tìm kiếm sách
const handleBookSearch = () => {
  ...
  searchState.results = availableBooks.filter(book => {
...
// hàm call api load danh sách yêu thích
...
// Hàm tải sản phẩm Flash Sale và render
    ...
    if (container) {
      if (Array.isArray(data) && data.length > 0) {
        container.innerHTML = data.map(product => {
          ...
          return `
            <div class="product-card relative group flex flex-col">
...
// ... (Giữ nguyên các hàm không thay đổi khác: loadReviewsForProduct, setBookDetailTab, handleBuyNow, Review Modals, updateQuantity, showMessage, Cart functions, Auth Pages, Profile Page, Order Tracking, Checkout, Place Order, Admin functions, v.v...)
// ... (Tất cả các hàm từ 'loadReviewsForProduct' trở đi có thể được giữ nguyên)
...
// Render trang quản lý bình luận
...
// Hàm render sao đánh giá (DÙNG CHUNG TOÀN CỤC)
